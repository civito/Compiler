...
/* reserved words */
%token ELSE IF INT RETURN VOID WHILE THEN END REPEAT UNTIL READ WRITE
...
saveNumber  : NUM
            ...
            ;
var_decla    : type_spec saveName SEMI
                 { $$ = newDeclNode(VarK);
                   $$->child[0] = $1; /* type */
                   $$->lineno = lineno;
                   $$->attr.name = savedName;
...
param       : type_spec saveName
            ...
            ;
comp_stmt   : LCURLY local_declas stmt_list RCURLY
                 { $$ = newStmtNode(CompK);
                   $$->child[0] = $2; /* local variable declarations */
                   $$->child[1] = $3; /* statements */
                 }
            ;
local_declas : local_declas var_decla
                 { YYSTYPE t = $1;
                   if (t != NULL)
                   { while (t->sibling != NULL)
                        t = t->sibling;
